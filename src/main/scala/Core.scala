...
class Core extends Module {
  ...
  val id_imm_b =
    Cat(
      ...
      id_inst(30, 25),
      id_inst(11, 8)
    ) // immedeate value for B-type instruction
  ...
  val id_imm_j =
    Cat(
      ...
      id_inst(20),
      id_inst(30, 21)
    ) // immedeate value for J-type instruction
  ...
  exe_alu_out := MuxCase(
    ...
    Seq(
      ...
      (exe_reg_exe_fun === ALU_SLT) -> (exe_reg_op1_data.asUInt() < exe_reg_op2_data.asUInt()).asUInt(), // signed
      (exe_reg_exe_fun === ALU_SLTU) -> (exe_reg_op1_data < exe_reg_op2_data).asUInt(), // unsigned
